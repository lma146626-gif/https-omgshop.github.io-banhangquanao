...
function addToCart(productId,inputQuantity = 1, selectedColor = null) {
    let product = products.find(p => p.id == productId);
    if (product) {
        let existingProduct = cart.find(p => p.id == productId && p.selectedColor === selectedColor);
        if (existingProduct) {
            existingProduct.quantity += inputQuantity;
        } else {
            let productWithQuantity = { ...product, quantity: inputQuantity };
            if (selectedColor) productWithQuantity.selectedColor = selectedColor;
            ...
            <div class="cart_product_info">  
                <div class="top_card">
                    <div class="left_card">
                        ...
                        ${product.selectedColor ? `<span class="product_color">Màu: ${product.selectedColor}</span>` : ''}
                    </div>
